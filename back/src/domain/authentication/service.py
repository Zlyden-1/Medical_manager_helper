...
from src.domain.authentication.exception import InvalidCredentials, AccessNotFound, AccessExpires, InvalidRole, \
    RefreshNotFound
...
async def validate_user_credentials(
    ...
    user = await UserDAO(session).get_by_email(credentials.email)
    if user is None:
        raise InvalidCredentials
